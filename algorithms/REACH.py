...
def use_reach(text):
    '''
    See the documentation for the Indra instance at https://buildmedia.readthedocs.org/media/pdf/indra/latest/indra.pdf
    Build the reach docker container on pages 51 to 52
    ...
    # Run reach
    rp = reach.process_text(text = text) # url = reach.local_text_url)
    ...
    if rp is not None:
        ...
        if len(rp.statements) > 0:

            # Hold all results
            theResults = []

            # Iterate through statements
            for num in range(len(rp.statements)):

                # Pull the first and second statement 
                first_stat = rp.statements[num].real_agent_list()[0].db_refs
                second_stat = rp.statements[num].real_agent_list()[1].db_refs
                ...
                if first_acc and second_acc:
                    ...
                    theResults.append([name_collapse(first_stat), name_collapse(second_stat), 1, "Indra Reach"])
